...
func TestChargementDesFixtures(t *testing.T) {
	...
	for _, niveau := range []string{"discovery", "investigation", "expert"} {
		liste, _ := List(db, "", niveau)
		if len(liste) != 2 {
			t.Error(niveau, ": attendu 2 enquetes, obtenu", len(liste))
		}
	}
...
func TestRechargementMetAJour(t *testing.T) {
	db := baseAvecEnquetes(t)

	// on simule une ancienne version de l'enquete en base
	db.Exec("UPDATE cases SET difficulty = 'discovery' WHERE slug = 'sql-dossiers-fantomes'")
	var id int64
	db.QueryRow("SELECT id FROM cases WHERE slug = 'sql-dossiers-fantomes'").Scan(&id)
	...
	c, err := Get(db, "sql-dossiers-fantomes")
	if err != nil {
		t.Fatal(err)
	}
	if c.Difficulty != "expert" {
		t.Error("le rechargement n'a pas mis a jour le niveau, obtenu", c.Difficulty)
	}
	// l'id ne doit pas changer, des investigations pointent dessus
	var idApres int64
	db.QueryRow("SELECT id FROM cases WHERE slug = 'sql-dossiers-fantomes'").Scan(&idApres)
	if idApres != id {
		t.Error("l'id de l'enquete a change :", id, "->", idApres)
	}
}

...
func LoadFixtures(db *sql.DB, chemin string) (int, error) {
	...
	for _, c := range liste {
		...
		// si le slug existe deja on met la ligne a jour : une modification
		// du fichier est prise en compte au prochain lancement, sans perdre
		// l'id (les enquetes en cours pointent dessus)
		_, err = db.Exec(`INSERT INTO cases (slug, language, difficulty, content_json, source)
			VALUES (?, ?, ?, ?, 'local')

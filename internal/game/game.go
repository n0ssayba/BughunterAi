...
// le temps ecoule depuis l'ouverture du dossier. C'est le serveur qui
// compte, le navigateur n'a que l'affichage (section 5.2)
func SecondesEcoulees(db *sql.DB, invID int64) (int, error) {
	var secondes int
	err := db.QueryRow(`SELECT CAST((julianday('now') - julianday(started_at)) * 86400 AS INTEGER)
		FROM investigations WHERE id = ?`, invID).Scan(&secondes)
	if err != nil {
		return 0, err
	}
	if secondes < 0 {
...
// le retour de l'assistant-detective, dans la forme de la section 7.2
...
type Rapport struct {
	...
	BonusTemps          int    `json:"bonus_temps"`
	// rempli par l'assistant-detective quand OpenRouter a repondu,
	// absent sinon : le rapport local reste complet (mode de secours F12)
	Commentaire *Commentaire `json:"commentaire,omitempty"`
}
...
func Verdict(db *sql.DB, userID int64, invID int64, suspectID string, fixID string) (bool, int, error) {
	...
	secondes, err := SecondesEcoulees(db, invID)
	if err != nil {
		return false, 0, err
	}
	score := Score(mauvaises, indices, fixChoisie.Correct, secondes)
	...
	rapport := Rapport{
		...
		Secondes:            secondes,
		BonusTemps:          BonusTemps(secondes),
	}
...
func AjouterCommentaire(db *sql.DB, invID int64, commentaire Commentaire) error {

...
func TestVerdictScoreComplet(t *testing.T) {
	...
	// 100 - 10 (une fausse piste) - 5 - 10 (deux indices) + 10 (chrono :
	// le test se resout instantanement)
	if score != 95 {
		t.Error("attendu 95, obtenu", score)
	}
}
...
func TestVerdictMauvaiseCause(t *testing.T) {
	...
	if !resolu || score != 110 {
		t.Error("attendu 110 (100 - 10 + 10 premier essai + 10 chrono), obtenu", score)
	}
}
...
func TestProgression(t *testing.T) {
	...
	Verdict(db, 1, id1, "A", "F1") // 100 + 10 premier essai + 10 chrono = 120
	...
	if p.ScoreTotal != 120 {
		t.Error("attendu un score total de 120, obtenu", p.ScoreTotal)
	}
}
...
func TestProgressionConceptsEtMeilleurScore(t *testing.T) {
	...
	if p.MeilleurScore != 120 {
		t.Error("attendu un meilleur score de 120, obtenu", p.MeilleurScore)
	}
	if len(p.Concepts) != 2 {

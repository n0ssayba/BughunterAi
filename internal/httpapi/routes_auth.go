...
func (s *Server) register(w http.ResponseWriter, r *http.Request) {
	...
	if len(ids.Username) < 3 || len(ids.Username) > 30 {
		repondreErreur(w, 400, "le nom doit faire entre 3 et 30 caracteres")
		return
	}
	// bcrypt refuse au-dela de 72 octets : on le dit clairement plutot que
	// de laisser partir une erreur serveur
	if len(ids.Password) < 8 || len(ids.Password) > 72 {
		repondreErreur(w, 400, "le mot de passe doit faire entre 8 et 72 caracteres")
		return
	}

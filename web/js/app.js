...
let assistantVerdict = '';
let chronoSecondes = 0;
let chronoMinuteur = null;
...
async function inscrire() {
  ...
  if (nom.length < 3 || nom.length > 30) {
    message('erreur', "L'identifiant doit faire entre 3 et 30 caractères.", 'message-inscription');
    return;
  }
  if (mdp.length < 8 || mdp.length > 72) {
    message('erreur', 'Le mot de passe doit faire entre 8 et 72 caractères.', 'message-inscription');
    return;
  }
  if (mdp !== mdp2) {
...
// le compteur affiche part du temps deja ecoule : en cas de reprise il ne
// recommence pas a zero. Le score, lui, est calcule par le serveur.
function demarrerChrono(secondes) {
  arreterChrono();
  chronoSecondes = secondes;
  document.getElementById('chrono').textContent = formaterTemps(chronoSecondes);
  chronoMinuteur = setInterval(function () {
    chronoSecondes = chronoSecondes + 1;
    document.getElementById('chrono').textContent = formaterTemps(chronoSecondes);
  }, 1000);
}

function arreterChrono() {
  if (chronoMinuteur !== null) {
    clearInterval(chronoMinuteur);
    chronoMinuteur = null;
  }
}
...
async function ouvrirDossier(slug, numero) {
  ...
  montrer('dossier');
  demarrerChrono(inv.donnees.secondes);
...
async function demanderIndice() {
  ...
  // les trois niveaux d'aide de la section 5.1
  const nomsNiveaux = { 1: 'Observation', 2: 'Direction', 3: 'Presque résolu' };
  const titre = 'Indice ' + r.donnees.level + ' · ' + nomsNiveaux[r.donnees.level];
  if (r.donnees.source === 'secours') {
    // l'IA etait la mais n'a pas repondu : on le dit, sans le detail technique
    message('info', "L'assistant-détective n'a pas répondu, voici l'indice du dossier. " +
      titre + ' — ' + r.donnees.hint);
  } else {
    message('indice', titre + ' — ' + r.donnees.hint);
  }
  const p = document.createElement('p');
  let ligne = titre + ' — ' + r.donnees.hint;
...
async function choisirCorrection(fixId) {
  ...
  arreterChrono();
  assistantVerdict = r.donnees.assistant;
...
async function chargerRapport() {
  ...
  document.getElementById('rapport-score').textContent = r.donnees.score + ' / 120';

  let temps = formaterTemps(r.donnees.secondes);
  if (r.donnees.bonus_temps > 0) {
    temps = temps + ' · bonus de rapidité +' + r.donnees.bonus_temps;
  }
  document.getElementById('rapport-temps').textContent = temps;
  ...
  const c = r.donnees.commentaire;
  if (c) {
    document.getElementById('ia-observation').textContent = c.observation;
    document.getElementById('ia-concept').textContent = c.concept;
    document.getElementById('ia-question').textContent = c.question;
    // pas d'erreur de raisonnement quand le joueur n'a pas fait de fausse piste
    document.getElementById('ia-erreur').textContent = c.erreur;
    document.getElementById('ia-erreur-ligne').className = c.erreur ? '' : 'cache';
    document.getElementById('rapport-ia').className = 'rapport-ia';
  } else {
    document.getElementById('rapport-ia').className = 'rapport-ia cache';
  }
  // l'assistant etait configure mais n'a pas repondu : on le dit (15.3)
  document.getElementById('rapport-ia-absent').className =
    (!c && assistantVerdict === 'secours') ? 'italique' : 'italique cache';
